...
From here, we need to make the assumption that \gam is in fact irrational. The justification for this is that if \gam is rational, then we can define it exactly as a quotient of integers, making the whole aim redundant. 

In this case we can see that if equation \ref{equalgammas} holds, then since \gam is irrational must be the case that both $\gamma_{i}$ and $\gamma_{i+1}$ are less than \gam, or they're both greater than \gam. Since if one was greater than \gam and one less, it would contradict \gam being irrational.
...
Given the argument in \emph{case 1}. We know that $\gamma_{i+1}$ will be closer to the real \gam than \gami, since otherwise it wouldn't be one of the sequence. Hence
...
This suggests we should be able to find
